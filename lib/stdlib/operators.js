...
function proc_plus(exps) {
	...
	while (exps.length > 0) {
		var shifted = exps.shift();
		try {
			value.add(shifted.coerce(value.type));
		} catch (e) {
			console.error(typeof shifted, shifted);
			console.error(exps);
			throw e;
		}
	}
...
function proc_minus(exps) {
	...
	var value = new ALVar(exps.shift()); // grab first
	while (exps.length > 0)
		value.sub(exps.shift().coerce(value.type));
	return value;
}
...
function proc_multiply(exps) {
	...
	var value = new ALVar(exps.shift()); // grab first
	while (exps.length > 0)
		value.mul(exps.shift().coerce(value.type));
	return value;
}
...
function proc_divide(exps) {
	...
	var value = new ALVar(exps.shift()); // grab first
	while (exps.length > 0)
		value.div(exps.shift().coerce(value.type));
	return value;
}
...
exports.proc_morethanequal = (exps) => {
	runtime.check(exps, runtime.has_two_arguments);
	var first = exps[0];
	var second = exps[1];
	return atoms.bool(first.value >= second.value);
