...
State.prototype.on = function(evt, cb) {
	this.events[evt] = cb;
	return this;
};

State.prototype.fire = function(evt, ...args) {
	if(!evt) throw 'fire: invalid event';
	if(typeof this.events[evt] === 'function')
		this.events[evt].apply(null, args);
	return this;
};

State.prototype.start = function() {
	this.fire(exports.events.start);
	return this;
};
...
state.on(exports.events.start, read);
...
function loop() {
	if(state.exit) {
		rl.close();
		state.fire(exports.events.close);
	} else {
		read();
	}
}
